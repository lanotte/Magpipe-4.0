...
int msg_handle(msg_t *msg,int connectfd)                     //处理客户端信息      
{
    ...
    if(LOG == msg->action)                                         //指令:登录
    {
        ...
        read_unlog_msg(msg->name,connectfd);
    ...
    else if(WHISPER == msg->action)                                         //指令:私聊
    {
        printf("正在进行私聊!\n");
        ...
        if(0 == strlen(str_return))                        //不在线 
        {
            ...
            time(&ptime);
            strcpy(pestime,ctime(&ptime));
            char sql_whsp_unlog_info[256];                 //保存离线消息
            sprintf(sql_whsp_unlog_info,"insert into unlog_info(name,target,msg,time) values('%s','%s','%s','%s')",msg->name,msg->target,msg->msg,pestime);
            rc= sqlite3_exec(db,sql_whsp_unlog_info,NULL,0,&err_msg);
        ...
        sprintf(sql_whsp_record, "insert into record (id,name,size,target,msg,time) values(%d,'%s','%s','%s','%s','%s')",msg->id,msg->name,"私聊",msg->target,msg->msg,pestime); 
    ...
    {   
        ...
        msg->action = GETOUT;                              //修改成功后自动下线，等同被离线  
